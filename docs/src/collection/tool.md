---
article: false
---

# 工具

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/leetcode.ico
  name: LeetCode
  desc: 全球极客挚爱的技术成长平台
  link: https://leetcode.cn/
  
- icon: https://img.sherry4869.com/blog/collection/tools/lintcode.ico
  name: LintCode
  desc: 通过在线实操的方式，提升编程能力
  link: https://www.lintcode.com/

- icon: https://img.sherry4869.com/blog/collection/tools/regexlearn.svg
  name: RegexLearn
  desc: 逐步学习正则表达式，从零基础到高阶
  link: https://regexlearn.com/zh-cn

- icon: https://img.sherry4869.com/blog/collection/tools/code.svg
  name: CODE
  desc: 任何人都可以学习计算机科学
  link: https://code.org/
```
:::

---

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/uutool.ico
  name: UU在线工具
  desc: 工欲善其事，必先利其器
  link: https://uutool.cn/

- icon: https://img.sherry4869.com/blog/collection/tools/pdf24.png
  name: PDF24 Tools
  desc: 免费且易于使用的在线PDF工具
  link: https://tools.pdf24.org/zh/
  
- icon: https://img.sherry4869.com/blog/collection/tools/unit.ico
  name: UNIT helper
  desc: 在线单位换算器
  link: https://cn.unithelper.com/

- icon: https://img.sherry4869.com/blog/collection/tools/greasyfork.png
  name: Greasy Fork
  desc: 提供用户脚本的网站
  link: https://greasyfork.org/zh-CN

- icon: https://img.sherry4869.com/blog/collection/tools/qmjianli.png
  name: 全民简历
  desc: 10分钟快速制作简历
  link: https://www.qmjianli.com
```
:::

---

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/processon.svg
  name: ProcessOn
  desc: 免费在线流程图思维导图
  link: https://www.processon.com/

- icon: https://img.sherry4869.com/blog/collection/tools/mindline.png
  name: MindLine
  desc: 快速高效制作思维导图工具
  link: https://www.mindline.cn/webapp
```
:::

---

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/duolingo.ico
  name: 多邻国
  desc: 好玩高效的英语学习途径
  link: https://www.duolingo.cn/

- icon: https://img.sherry4869.com/blog/collection/tools/deepl.svg
  name: DeepL 翻译器
  desc: 全世界最准的翻译
  link: https://www.deepl.com/translator
```
:::

---

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/pagespeed.png
  name: PageSpeed Insights
  desc: 使您的网页在所有设备上都能快速加载
  link: https://pagespeed.web.dev/
```
:::

---

::: projects
```yaml
- icon: https://img.sherry4869.com/blog/collection/tools/squoosh.png
  name: Squoosh
  desc: Small Simple Secure
  link: https://squoosh.app/
```
:::