...
如果你对虚拟 DOM 的概念比较熟悉，并且偏好直接使用 JavaScript，你也可以结合可选的 JSX 支持[直接手写渲染函数](../extras/render-function.md)而不采用模板。但请注意，这将不会享受到和模板同等级别的编译时优化
...
双大括号标签会被替换为[相应组件实例](reactivity-fundamentals.md#声明响应式状态)中 `msg` 属性的值。同时每次 `msg` 属性更改时它也会同步更新

## 原始 HTML

双大括号会将数据解释为纯文本，而不是 HTML。若想插入 HTML，你需要使用 [v-html](../../api/built/directives.md#v-html) 指令：
...
双大括号不能在 HTML attributes 中使用。想要响应式地绑定一个 attribute，应该使用 [v-bind 指令](../../api/built/directives.md#v-bind)：
...
没有显式包含在列表中的全局对象将不能在模板内表达式中访问，例如用户附加在 `window` 上的属性。然而，你也可以自行在 [app.config.globalProperties](../../api/global/application.md#app-config-globalproperties) 上显式地添加它们，供所有的 Vue 表达式使用
...
![](https://mw-blog.oss-cn-guangzhou.aliyuncs.com/blog/it/vue/img.png)