...
- Dev (Development) 环境：这是开发人员工作的地方，是代码最初的开发环境。在这个环境下，开发人员可以编写、测试和修改软件代码，然后将其提交到版本控制系统，以供其他团队成员查看和协作
...
2. 修改 pom.xml 文件
    ...
    需要注意的是：在 Maven 中，src/main/resources 目录下的资源文件会默认被打包到最终的构建文件中，例如 JAR 或 WAR 文件。在这个过程中，Maven 会尝试对这些资源文件进行过滤，以解析其中的占位符（例如 ${env}）或者通过 Maven Profile 将不同的环境配置文件打包到不同的构建文件中。
